...
class PlotterActor(OperatorActor):
    ...
    def on_start(self):
        self._register(self.plotter.location,
                       {const.UNIT_TYPE_KEY: const.PLOTTER_TYPE, const.COLOR_KEY: self.plotter.color})

    def on_receive(self, message):
        if isinstance(message, PlotMessage):  # Blocking Request
            self.plotter.plot()
